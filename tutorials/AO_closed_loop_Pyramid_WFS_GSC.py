# -*- coding: utf-8 -*-
"""
Created on Fri Nov 14 10:04:18 2025

@author: cheritier


This tutorial demonstrates how to build a complete adaptive optics (AO) simulation in OOPAO, 
with a specific focus on using the Gain Sensing Camera (GSC) to measure and compensate optical gains in closed loop. 
After setting up the telescope, guide star, atmosphere, deformable mirror, and Pyramid WFS, KL modal basis, 
the Gain Sensing Camera is introduced to estimate mode-by-mode optical gains directly from focal-plane intensity variations.

The tutorial shows how to:

- Initialize the GSC with the WFS mask and KL modal basis
- Perform its calibration
- Use the GSC to retrieve optical gains (gsc.og) for a given turbulent input

A full closed-loop simulation is then executed, where the GSC continuously updates the optical gain estimate at each frame,
improving loop stability and maintaining performance in the presence of varying seeing conditions and temporal effects. 

The tutorial tracks turbulence, residual wavefront error, and Strehl ratio to illustrate the impact of optical-gain compensation.
