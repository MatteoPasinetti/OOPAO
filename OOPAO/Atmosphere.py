...
class Atmosphere:
    ...
    def fill_scintillation_support(self, tmp_layer, scintillation_support, distance, i_layer):
        ...
        for i_src, src in enumerate(self.src_list):
            ...
            if distance > 1e-6:
                scintillation_support[i_src] = self.angular_spectrum_method(
    ...
    @property
    def rytov_var(self):
        if not getattr(self, 'angular_spectrum_method', False):
            return 0.0
    ...
    def rytov_var(self, val):
        if not getattr(self, 'angular_spectrum_method', False):
            warning("Cannot set Rytov variance because atm.angular_spectrum_method is False.")
            return    
        val = float(val)
        if val <= 0:
            return 
        ...
        if getattr(self, 'angular_spectrum_method', False):
